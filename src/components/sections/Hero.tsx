      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
        ...
        <div className="lg:col-span-5 flex flex-col items-start z-10">
          ...
          <p className="hero-text text-base lg:text-lg text-secondary leading-relaxed mb-8 max-w-md">
            ACAS brings document ingestion, financial analysis, verification, evidence and credit review into one structured workflow for modern lenders.
          </p>
          
          <div className="hero-text flex flex-col sm:flex-row items-start sm:items-center gap-6">
            <Link href="/demo" className="bg-brand text-white px-6 py-3 text-sm font-semibold hover:bg-brand/90 transition-colors flex items-center gap-2 group">
              Request Early Access
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </Link>
            <Link href="/features" className="text-sm font-semibold text-brand hover:text-primary transition-colors underline underline-offset-4 decoration-border hover:decoration-primary">
        ...
        <div className="lg:col-span-7 relative h-[400px] sm:h-[450px] lg:h-[500px] w-full perspective-[1000px] z-0">
          ...
          <div className="doc-layer absolute top-[5%] right-[20%] w-64 bg-white border border-border shadow-lg p-4 rotate-[6deg] opacity-70 origin-bottom-right">
            <div className="text-[10px] text-secondary/50 font-mono mb-4 border-b border-border/50 pb-2">GSTR-3B_FY25.PDF</div>
            <div className="space-y-2">
              <div className="h-2 bg-border/40 rounded w-full" />
              <div className="h-2 bg-border/40 rounded w-5/6" />
              <div className="h-2 bg-border/40 rounded w-4/6" />
            </div>
          </div>

          {/* Document Layer 2: Bank Statement */}
          <div className="doc-layer absolute top-[15%] right-[45%] w-72 bg-white border border-border shadow-xl p-4 -rotate-[3deg] opacity-90 origin-bottom-left">
            <div className="text-[10px] text-secondary/50 font-mono mb-4 border-b border-border/50 pb-2">HDFC_STATEMENT_Q4.PDF</div>
            <div className="space-y-3">
              <div className="flex justify-between items-center"><div className="h-2 bg-border/60 rounded w-16"/><div className="h-2 bg-border/40 rounded w-12"/></div>
              <div className="flex justify-between items-center"><div className="h-2 bg-border/60 rounded w-20"/><div className="h-2 bg-border/40 rounded w-10"/></div>
              <div className="flex justify-between items-center"><div className="h-2 bg-border/60 rounded w-14"/><div className="h-2 bg-border/40 rounded w-16"/></div>
            </div>
          </div>

          {/* Core UI Panel: ACAS Credit Case */}
          <div className="ui-panel absolute top-[30%] left-[5%] right-[5%] sm:left-[15%] sm:right-[15%] bg-white border border-border shadow-2xl rounded-sm overflow-hidden z-20">
            ...
            <div className="bg-brand text-white px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold tracking-widest uppercase">ACAS Credit Case</span>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1 border border-border p-3 flex items-center justify-between bg-background">
                  <span className="text-xs font-bold text-secondary">GST ↔ BANK</span>
                  <span className="flex items-center gap-1 text-[10px] font-bold bg-amber-100 text-amber-800 px-2 py-0.5 rounded-sm uppercase tracking-wide">
                    Review
                  </span>
                </div>
                <div className="flex-[2] border border-border p-3 flex items-center gap-3 bg-background">
                  <FileText className="w-4 h-4 text-secondary/50" />
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-secondary">EVIDENCE: REVENUE</span>
                    <span className="font-mono text-[10px] text-brand">GSTR-3B_FY25.pdf (Page 4)</span>
                  </div>
                </div>
              </div>
